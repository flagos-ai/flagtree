...
class TLESemantic:
        ...
        for i, dim in enumerate(shape):
            if hasattr(dim, 'value'):  # constexpr-like objects
                ...
                raise TLESemanticError(
                    f"Shape dimension {i} must be integer or constexpr-like, but got {type(dim)}",
                    "alloc"
                )

            if dim_val <= 0:
                raise TLESemanticError(
                    f"Shape dimension {i} must be positive, but got {dim_val}",
                    "alloc"
                )
    ...
    def validate_alloc_dtype(self, dtype: tl.dtype) -> tl.dtype:
        ...
        if not isinstance(dtype, tl.dtype):
            raise TLESemanticError(
                f"Data type must be tl.dtype, but got {type(dtype)}",
                "alloc"
            )
        ...
        supported_types = [
            ...
            tl.int1  # boolean type equivalent in Triton
        ]

        if dtype not in supported_types:
            warnings.warn(
                f"Data type {dtype} may not be fully supported, recommend using standard data types",
                UserWarning
            )
    ...
    def validate_copy_compatibility(self, src: tl.tensor, dst: tle.buffered_tensor,
        ...
        for i, copy_dim in enumerate(copy_shape_unwrapped):
            if i < len(src_shape) and copy_dim > src_shape[i]:
                raise TLESemanticError(
                    f"Copy dimension {i} ({copy_dim}) exceeds source tensor range ({src_shape[i]})",
                    "copy"
                )
            if i < len(dst_shape) and copy_dim > dst_shape[i]:
                raise TLESemanticError(
                    f"Copy dimension {i} ({copy_dim}) exceeds destination buffer range ({dst_shape[i]})",
                    "copy"
                )
    ...
    def validate_local_load_buffer(self, buffer: tle.buffered_tensor) -> None:
        ...
        if not isinstance(buffer, tle.buffered_tensor):
            raise TLESemanticError(
                f"Buffer must be tle.buffered_tensor, but got {type(buffer)}",
                "local_load"
            )


    def analyze_alloc_operation(self, shape: Sequence[Union[int, any]],
                              dtype: tl.dtype, layout: Optional[tle.shared_layout],
                              storage: tle.scope) -> Tuple[List[int], tl.dtype]:
