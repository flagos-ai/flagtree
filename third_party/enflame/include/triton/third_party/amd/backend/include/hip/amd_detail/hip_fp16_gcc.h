...
inline unsigned short __internal_float2half(float flt, unsigned int &sgn,
                                            unsigned int &rem) {
  unsigned int x{};
...
inline __half __float2half(float x) {
  ...
  if (rem > 0x80000000U || (rem == 0x80000000U && (r.x & 0x1)))
    ++r.x;
...
inline __half2 __float2half2_rn(float x) {
...
inline float __half2float(__half x) {
