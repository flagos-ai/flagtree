...
typedef struct {
  ...
  uint64_t dispatch_id; // Dispatch ID of the AQL packet that needs more scratch
                        // memory
...
typedef union {
  ...
  const hsa_amd_event_scratch_async_reclaim_start_t
      *scratch_async_reclaim_start;
