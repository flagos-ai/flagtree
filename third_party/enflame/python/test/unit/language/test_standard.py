...
def test_sort(M, N, descending, dtype_str, device):
    ...
    sort_kernel[(1, )](x, z, N, M, descending, num_warps=4)
    assert (y == z).all(), (y, z)
...
def test_flip(M, N, dtype_str, device):
    ...
    x = numpy_random((N, M), dtype_str=dtype_str)
    x = torch.from_numpy(x).to(device)
    # TODO(chongzhou.yang): torch.flip is not yet supported in gcu400, fallback it to cpu until 2025.12.31
    y = torch.flip(x.to('cpu'), (1, ))
    z = torch.empty_like(x, device=device)
    # num_warps in gcu4xx can not exceed 4
    flip_kernel[(1, )](x, z, N, M, num_warps=4)
    assert (y.to('gcu') == z).all(), (y, z)
