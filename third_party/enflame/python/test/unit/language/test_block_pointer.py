...
@pytest.mark.parametrize("dtype_str, n, padding_option", [(dtype_str, n, padding)
                                                          for dtype_str in ("bool", "int16", "float16")
                                                          for n in (64, 128, 256, 512, 1024)
                                                          for padding in ("zero", "nan")])
def test_block_copy(dtype_str, n, padding_option, device):
    ...
    if dtype_str in ("bool", "int16"):
        ...
        a = torch.randint(0, 2, (n, ), device=device, dtype=dtype)
    else:
...
@triton.jit
def matmul_no_scf_with_advance_kernel(a_ptr, b_ptr, c_ptr, M, N, K, stride_am, stride_ak, stride_bk, stride_bn,
...
@pytest.mark.parametrize("shape, num_warps", [(shape, num_warps) for shape in [
    [64, 64, 16],
    [64, 64, 32],
    [64, 64, 64],
...
def test_block_ptr_matmul_no_scf(shape, num_warps, device):
    ...
    grid = lambda META: (1, )
    matmul_no_scf_with_advance_kernel[grid](a_ptr=a, b_ptr=b, c_ptr=c, M=m, N=n, K=k, stride_am=a.stride(0),
                                            stride_ak=a.stride(1), stride_bk=b.stride(0), stride_bn=b.stride(1),
                                            stride_cm=c.stride(0), stride_cn=c.stride(1), BLOCK_M=m, BLOCK_N=n,
                                            BLOCK_K=k, num_warps=num_warps)
