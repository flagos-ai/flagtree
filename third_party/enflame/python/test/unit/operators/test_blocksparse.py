...
def make_pair(shape, device="gcu", alpha=1e-2, beta=0., trans=False, data=None, dtype=torch.float32):
    if data is None:
...
# TODO(rickon.wang): MODE "dds" sip hang. backward not working. param 'BLOCK" is
# set 64 due to runtime limit.
@pytest.mark.parametrize("MODE", ["sdd", "dsd"])
...
def test_matmul(MODE, TRANS_A, TRANS_B, BLOCK, DTYPE, Z=3, H=2, M=512, N=384, K=256):
    ...
    # a_tri.retain_grad()
    # b_tri.retain_grad()
    op = triton.ops.blocksparse.matmul(layout, BLOCK, MODE, trans_a=TRANS_A, trans_b=TRANS_B, device="gcu")
...
def test_softmax(BLOCK, WIDTH, is_dense, Z=2, H=2, is_causal=True, scale=0.4):
    ...
    a_ref.retain_grad()
    at_mask = torch.ones((M, N), device="gcu")
    if is_causal:
    ...
    op = triton.ops.blocksparse.softmax(layout, BLOCK, device="gcu", is_dense=is_dense)
...
#TODO(rickon.wang): backward disabled. runtime limit.
...
def test_attention_fwd_bwd(
    ...
    capability = torch.gcu.get_device_capability()
    # if capability[0] < 7:
    ...
    qkvs = [
        torch.nn.Parameter(input_scale * torch.randn(qkv_shape), requires_grad=True).to(dtype).gcu() for _ in range(3)
    ]
    ...
    attn_mask = torch.ones([n_ctx, n_ctx], device="gcu", dtype=dtype)
    attn_mask = torch.tril(attn_mask, diagonal=0)
    attn_mask = 1e6 * (-1 + (attn_mask.reshape((1, 1, n_ctx, n_ctx)).gcu()))
    # torch_q.retain_grad()
    # torch_k.retain_grad()
    # torch_v.retain_grad()
