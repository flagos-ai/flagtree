...
createSchedule(scf::ForOp forOp, int numStages) {
  ...
  for (Operation &op : forOp.getBody()->without_terminator()) {
    if (isa<mlir::triton::gcu::AsyncLoadGlobalToShareOp>(op))
...
static void hoistAllocAndConst(scf::ForOp forOp) {
  ...
  for (Operation &op : forOp.getBody()->without_terminator()) {
    if (auto allocOp = dyn_cast<mlir::triton::gcu::ShareAllocOp>(op)) {
  ...
  for (Operation *op : toHoist) {
    op->moveBefore(forOp);
    auto allocOp = dyn_cast<mlir::triton::gcu::ShareAllocOp>(op);
    if (!allocOp)
      continue;
    for (Operation *user : allocOp->getUsers()) {
      if (auto dealloc = dyn_cast<mlir::triton::gcu::ShareDeallocOp>(user)) {
