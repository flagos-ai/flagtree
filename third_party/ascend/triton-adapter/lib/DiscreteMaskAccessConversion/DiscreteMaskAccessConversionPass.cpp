...
struct DiscreteMaskAtomicAddConversion : OpRewritePattern<triton::AtomicRMWOp> {
  ...
  LogicalResult matchAndRewrite(triton::AtomicRMWOp op,
    ...
    if (op.getAtomicRmwOp() != triton::RMWOp::FADD &&
        op.getAtomicRmwOp() != triton::RMWOp::ADD) {
      return failure();
    }
    auto loc = op.getLoc();
    auto ptr = op.getPtr();
    ...
    if (auto tensorType = mlir::dyn_cast<mlir::RankedTensorType>(valueType)) {
      ...
      zeros = rewriter.create<mlir::arith::ConstantOp>(loc, denseAttr);
               ...
               mlir::isa<mlir::IntegerType>(valueType)) {
    ...
    auto maskedValue =
        rewriter.create<arith::SelectOp>(loc, mask, value, zeros);
    auto newAtomicAddOp = rewriter.create<triton::AtomicRMWOp>(
        loc, value.getType(), op.getAtomicRmwOp(), ptr, maskedValue,
